...
public class DefaultApplicationContext implements ApplicationContext {
    ...
    public DefaultApplicationContext(Hashtable definitions, Hashtable singletons, Vector processors) {
    ...
    public DefaultApplicationContext(int definitionsSize, int singletonsSize, int processorsSize) {
    ...
    public Vector getBeansOfType(Class type) throws ApplicationNotStarted, NoneBeanDefinitionFound {
        ...
        Vector allOfType = new Vector(all.size());
        for (int i = 0, size = all.size(); i < size; i++) {
            if (((BeanDefinition) all.items[i]).type.equals(type)) {
                allOfType.addElement(getBean(((BeanDefinition) all.items[i]).name));
            }
        }
        return allOfType;
    }
    ...
    private void callDestroyables() {
        Object any = null;
        for (int i = 0, size = singletons.size(); i < size; i++) {
            any = singletons.get(i);
            if (DestroyableBean.class.isInstance(any)) {
